...
system_input = """
你是一位优秀的数据分析助手，你能对输入文本进行预处理及体征提取并理解文本内容。请你精确识别出文本中包含的实体类型，请你不要遗漏实体类型，如果你遗漏实体类型，你将受到严厉的惩罚。请你以指定的格式返回结果（必须按照指定的格式进行输出），指定的格式为[{\"个人电话号码\":[\"13889871342\",\"13889871342",\"13889871342\"]},{\"个人姓名\": [\"张三\","\王五\","\李四\",\"张三\",\"刘七\"]},{\"IP地址\":[\"127.0.0.1\"]},...](注意：返回的是标准的json格式)）。请你对输出结果不能进行总结，请你不要胡编乱造，请你只能以指定的格式进行输出，否则你将受到惩罚。请你对未预定义的实体类型在输出结果中一定不能输出，如果输出你将受到惩罚。请你严格按照预定义的实体类别进行识别，请你确保不会自行创建定义中不存在的实体类别，并确保没有任何遗漏或错误。如果你未严格按照要求进行工作，你将会受到严重的处罚，以下是预定义的实体类型：
###
个人姓名：例如张三、李四、王五、赵六。
...
###
"""
use_input = "'张鹏,马云,110101234567890123456788,男,女,其他,未公开,工程学士,华为,420281199323322214'"

_input = "{'system':'" + system_input + '\',user:' + use_input + "}"

tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained(model_path, trust_remote_code=True).half().to('mps')

# 可指定不同的生成长度、top_p等相关超参
model.generation_config = GenerationConfig.from_pretrained(model_path, trust_remote_code=True)

# 第一轮对话
response, history = model.chat(tokenizer, _input, history=None)
print(response)
# 你好！很高兴为你提供帮助。
